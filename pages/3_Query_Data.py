"""
🔍 Query Data - Interactive Search & Download
Filter data and download as CSV
"""
...
st.title("🔍 Query Data")
st.markdown("Select filters, view data, and download as CSV")

tab1, tab2, tab3, tab4 = st.tabs(["📅 Orders", "🏷️ Products", "👥 Customers", "🏪 Sellers"])
...
with tab1:
    ...
    st.download_button(f"📥 Download {len(month_data):,} orders", csv, f"orders_{selected_month}.csv", "text/csv")
...
with tab2:
    ...
    cat_data = dim_products[dim_products['product_category_name'] == selected_cat][
    ...
    st.download_button(f"📥 Download {len(cat_data):,} products", csv, f"products_{selected_cat.replace(' ', '_')}.csv", "text/csv")
...
with tab3:
    ...
    state_data = dim_customers[dim_customers['state'] == selected_state][
    ...
    st.download_button(f"📥 Download {len(state_data):,} customers", csv, f"customers_{selected_state}.csv", "text/csv")
...
with tab4:
    ...
    col1, col2, col3 = st.columns(3)
    col1.metric("Showing", f"{len(sellers):,}")
    ...
    st.download_button(f"📥 Download top {len(sellers)} sellers", csv, "top_sellers.csv", "text/csv")
